...
import PassengerVerifyCash from '../../../src/actions/menu/passenger/verify-cash';
import { i18n } from '../../spec-support';

const user = {};

test('can be constructed with default parameters', t => {
  new PassengerVerifyCash({ i18n, user });
  t.pass();
});

test('should return composite response on get', t => {
  const action = new PassengerVerifyCash({ i18n, user });
  const response = action.get();
  t.is(response.type, 'composite');
  t.is(response.responses[0].type, 'text');
  t.is(response.responses[0].message, i18n.__('passenger-verify-cash.verify_cash'));
...
test('should return composite response on post', t => {
  ...
  t.is(response.responses[0].ok.responses[1].path, 'passenger-request-location');
  ...
  t.is(response.responses[1].ok.responses[0].message, i18n.__('passenger-verify-cash.get_some_cash'));
  t.is(response.responses[1].ok.responses[1].type, 'redirect');
  t.is(response.responses[1].ok.responses[1].path, 'passenger-index');
